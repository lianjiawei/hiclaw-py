...
export function getLoadedCharacterCount() {
...
export function getCharacterSprites(paletteIndex, hueShift = 0) {
    ...
    if (loadedCharacters && loadedCharacters.length > 0) {
        ...
        const walkSet = [empty, empty, empty, empty];
        const pairSet = [empty, empty];
        sprites = {
            walk: {
                [Dir.DOWN]: walkSet,
                [Dir.UP]: walkSet,
...
function hueShiftSprites(sprites, hueShift) {
    ...
    const shift = (sprite) => adjustSprite(sprite, color);
    const shiftWalk = (arr) => [shift(arr[0]), shift(arr[1]), shift(arr[2]), shift(arr[3])];
    const shiftPair = (arr) => [shift(arr[0]), shift(arr[1])];
    return {
        walk: {
...
export function decodeCharacterImage(image) {
    const down = [];
    const up = [];
    const right = [];
