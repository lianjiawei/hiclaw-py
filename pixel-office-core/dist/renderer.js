...
function getBubbleTone(ch) {
    if (ch.bubbleType === 'permission')
        return { accent: '#cca700', shadow: 'rgba(204, 167, 0, 0.2)' };
    if (ch.bubbleType === 'waiting')
        return { accent: '#44bb66', shadow: 'rgba(68, 187, 102, 0.18)' };
    if (ch.statusText?.includes('完成'))
        return { accent: '#44bb66', shadow: 'rgba(68, 187, 102, 0.16)' };
    if (ch.currentTool)
        return { accent: '#7c9bff', shadow: 'rgba(124, 155, 255, 0.16)' };
    return { accent: '#9aa7b9', shadow: 'rgba(154, 167, 185, 0.14)' };
}
function drawScrollingText(ctx, text, x, y, maxWidth, speed = 1) {
...
export function renderFrame(ctx, canvasWidth, canvasHeight, tileMap, furniture, characters, zoom, panX, panY, tileColors, cols, rows) {
...
function renderScene(ctx, furniture, characters, offsetX, offsetY, zoom) {
    const drawables = [];
...
function renderBubbles(ctx, characters, offsetX, offsetY, zoom) {
    for (const ch of characters) {
        if (!ch.bubbleType && !ch.statusText && !ch.statusDetail)
            continue;
...
function renderStatusBubble(ctx, ch, offsetX, offsetY, zoom) {
    const sittingOff = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0;
    ...
    if (detailLine) {
        ...
        ctx.font = `${detailFont}px monospace`;
        drawScrollingText(ctx, detailLine, x + padding + Math.round(3 * scale), y + padding + Math.round(11 * scale), width - padding * 2 - Math.round(3 * scale), 0.62);
    }
    ctx.restore();
}
