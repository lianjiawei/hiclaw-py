...
import { CharacterState, Direction, TILE_SIZE } from './exports-shim.js';

const READING_TOOLS = new Set(['Read', 'Grep', 'Glob', 'WebFetch', 'WebSearch']);

export function createCharacter(id: number, palette: number, seatId: string | null, seat: Seat | null, hueShift = 0): Character {
...
export function updateCharacter(ch: Character, dt: number, walkableTiles: Array<{ col: number; row: number }>, seats: Map<string, Seat>, tileMap: TileTypeVal[][], blockedTiles: Set<string>): void {
  ch.frameTimer += dt;
  switch (ch.state) {
...
export function getCharacterSprite(ch: Character, sprites: CharacterSprites): SpriteData {
  switch (ch.state) {
    case CharacterState.TYPE:
      return isReadingTool(ch.currentTool) ? sprites.reading[ch.dir][ch.frame % 2] : sprites.typing[ch.dir][ch.frame % 2];
    case CharacterState.WALK:
