...
import type { ColorValue, Direction, FurnitureInstance, OfficeLayout, PlacedFurniture, Seat, TileType as TileTypeVal } from './types.js';
...
export function getBlockedTiles(furniture: PlacedFurniture[], excludeTiles?: Set<string>): Set<string> {
  ...
  for (const item of furniture) {
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      ...
      for (let dc = 0; dc < entry.footprintW; dc++) {
        const key = `${item.col + dc},${item.row + dr}`;
        if (!excludeTiles || !excludeTiles.has(key)) tiles.add(key);
      }
    }
  }
  return tiles;
